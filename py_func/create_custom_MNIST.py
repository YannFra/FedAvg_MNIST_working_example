...
def save_dataset(dataset_name:str, array_X:np.array, array_y:np.array):
    ...
    with open(f'data/MNIST_custom/{dataset_name}.pkl', 'wb') as output:
        dataset = array_X, array_y
        pickle.dump(dataset, output)
        
        
        
def build_dataset(C:dict, std_size=2.5, dataset_digits=[i for i in range(10)]):
    ...
    if os.path.isfile(f"data/MNIST_custom/{file_name}.pkl"):
        return pickle.load(open(f"data/MNIST_custom/{file_name}.pkl", "rb"))
    ...
    pickle.dump(dataset, open(f'data/MNIST_custom/{file_name}.pkl', 'wb'))
...
def MNIST_custom_train_test_sets(clients, batch_size=10, shuffle=True):
